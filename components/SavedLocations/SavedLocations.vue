  ...
  <div class="saved-locations glass-dark" v-if="history.length > 0">
    <div class="saved-locations-grid">
      <div
          ...
          :class="{
            'current-location': item.isCurrentLocation,
            'active': currentLocationId === item.id
          }"
      ...
      >
        ...
        <div class="location-actions">
          ...
          <div class="dropdown-menu" v-if="activeDropdown === item.id">
            ...
            <!-- Remove option for regular locations and non-permanent current locations -->
            <button v-if="!item.isCurrentLocation || (item.isCurrentLocation && !item.isPermanent)" class="dropdown-item" @click.stop="$emit('remove', item.id, $event)">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M3 6h18"></path>
